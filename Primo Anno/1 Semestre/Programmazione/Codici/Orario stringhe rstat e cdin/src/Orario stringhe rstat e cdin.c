...
struct Orario AllocaColonne(struct Orario Orario1,int Colonne1){
	int pos=0;
	while (pos<LeggereRighe(Orario1))
	{
		Orario1.Tabella[pos]=malloc(Colonne1*sizeof(struct Lezione));
		pos=pos+1;
	}
	return Orario1;
};
...
int main(void) {
	...
	Orario1= AllocaColonne(Orario1,Colonne);
